...
# documented as a "safe fallback" — but it was unreferenced at runtime and used
# literal ``{"allow": False, ...}`` returns that the NEVER-DENY check below
# rejects, so calling ``compile_scope_fn(DEFAULT_SCOPE_FN_SOURCE)`` would raise.
# The tracked-run pipeline now fails closed when the scope agent does not
# produce a usable scope_fn (see ``Pipeline.run_query_agent_tracked``), so a
# silent fallback is no longer a feature we want.
...
def compile_scope_fn(source: str) -> Callable[[str, list, list[dict]], dict]:
    ...
    for node in ast.walk(tree):
        ...
        # NEVER-DENY soft constraint (iter 17 experiment):
        # Reject any scope_fn that statically returns ``{"allow": False, ...}``.
        # The privacy boundary is at the ROWS, not at the SQL — we want scope
        # to write row-transforming code instead of SQL-shape gates. A scope_fn
        # that wants to "block" a query should instead return
        # ``{"allow": True, "rows": [{"match_count": len(rows)}]}`` (aggregate)
        # or ``{"allow": True, "rows": <redacted>}`` (mask identifying fields).
        #
        # NOTE: this is an authoring-style hint, not a security boundary. A
        # determined scope_fn can still compute a deny via variable indirection
        # (``allow_val = False; return {"allow": allow_val, ...}``) and
        # ``apply_scope_fn`` honors it — see ``test_runtime_deny`` in
        # ``tests/test_scope.py`` for the explicit contract. Treat the static
        # check below as a nudge, not as a guarantee that scope cannot deny.
