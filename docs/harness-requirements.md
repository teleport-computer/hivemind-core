...
- **三类运行时 agent**，都是完整 agent（不是单条 SQL 发射器），各自带 prompt + 工具循环 + 多轮推理能力：
  ...
  - **Mediator agent**：只看 query 输出和 policy，负责最终输出审查；不拥有数据工具
...
- **三类运行时 agent 都需要被评测**，不只是 query。scope agent 自己的输出（scope_fn 源码、中间推理）和 mediator 的改写/放行行为都可能泄漏或损害效用
...
- artifact / generated-code 持久化：query 把原始内容写进 artifact 或代码常量——必须独立的 artifact / trajectory grader
...
7. **不要忽视 artifacts 和生成代码**。最终输出干净不代表 query 没把敏感值写进持久化文件或代码常量。
...
7. **Artifact / generated-code trajectory 评测**：写持久化是另一类威胁，需要专门的 grader 设计。
