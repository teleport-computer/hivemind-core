...
to produce `bench/scenarios_real.json` — used as a **held-out
...
mkdir -p bench/sources/privaci_bench
...
mkdir -p bench/sources/confaide
...
```bash
uv run python -m bench.sources.adapter
```
